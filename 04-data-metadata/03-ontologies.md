# Data/Metadata Layer — Ontologies
...
Relationships may reference anchors owned by other layers (System
Architecture, Infrastructure) where a genuine cross-layer relationship
exists. Per the reverse-pointer rule in 00-framework/relation-types.md,
every cross-layer relationship below has a matching reverse pointer added
to the target layer's ontology file.
...
- `data-custodian` --[requires]--> `sysarch-database`
  Rationale: cross-layer — a custodian's technical storage/security responsibility is exercised over an actual database.
...
- `data-quality-rule` --[constrains]--> `sysarch-database`
  Rationale: cross-layer — quality rules constrain what data is considered valid within a database.
...
- `data-technical-metadata` --[requires]--> `sysarch-database-schema`
  Rationale: cross-layer — technical metadata describes a data asset's structure, which for a database is its schema.
...
- `data-dataset` --[located-in]--> `sysarch-database`
  Rationale: cross-layer — a dataset is commonly a defined collection stored within a database.
- `data-business-glossary` --[contains]--> `data-glossary-term`
  Rationale: a glossary is composed of individual defined terms.
- `data-dictionary` --[requires]--> `sysarch-database-schema`
  Rationale: cross-layer — a data dictionary documents the fields of a specific schema.
- `data-semantic-layer` --[requires]--> `data-business-glossary`
  Rationale: a semantic layer maps technical fields to business-friendly names drawn from the glossary.
...
- `data-logical-model` --[produces]--> `sysarch-database-schema`
  Rationale: cross-layer — a logical model is implemented as a physical schema (System Architecture's Database Schema).
...
- `data-archival` --[requires]--> `infra-archive`
  Rationale: cross-layer — data archival is realized physically through Infrastructure's Archive storage tier.
...
- [x] Simple — 58 relationships, grouped by subdomain, no forced edges
- [x] Modular — cross-layer edges (6 total) are clearly marked and reverse-pointed
- [x] Easy to update — anchor-ID based, survives renames
- [x] Easy to maintain — grouped by subdomain, matches Definitions/Taxonomy
- [x] Easy to replace — relation-type vocabulary shared across all layers
