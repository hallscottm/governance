# Data/Metadata Layer — Metadata Standards
...
Same shape as the three prior layers (shared field registry + entity-type
applicability matrix). Worth noting explicitly: this column is somewhat
self-referential — this layer's own subject is metadata, so this column
...
- Sensitivity Level is Required for every entity type by design — it is
  the field this whole layer exists partly to finally give the
...
- Enforcement (mandatory-blocking vs. recommended) deferred to Policies
  (column 6), same boundary as all three prior layers.
- No field-level anchors, same framework-level gap flagged in all three
  prior layers' Metadata Standards.
...
- [x] Easy to replace — tool-agnostic, ISO/IEC 11179-grounded (same standard already cited by all three prior layers, now cited formally in this layer's own Standards Alignment)
