# Data/Metadata Domain — Standards Alignment
...
Scope per 00-framework/domain-axis-definition.md: what the data means —
taxonomy, ontology, catalog — explicitly not the databases/applications
that store and serve it (System Architecture, domain 3) or how it moves
through pipelines (Workflow/Process, domain 7). This is also where two
...
|---|---|---|
| **DAMA-DMBOK (Data Management Body of Knowledge)** | DAMA International | Comprehensive data management framework — data governance, quality, metadata management, master/reference data. The primary grounding reference for this domain's vocabulary. |
| **ISO/IEC 11179 (Metadata Registries)** | ISO/IEC | Formal standard for metadata registries and data element definitions — already used as informal grounding for every prior domain's Metadata Standards column; cited formally here since this domain governs metadata itself. |
...
| **NIST SP 800-60** | NIST | Guide for mapping information types to security categories — directly informs this domain's contribution to the Data-sensitivity Risk Tier factor left unresolved in Infrastructure, Networking, and System Architecture. |
| **NIST FIPS 199** | NIST | Security categorization standard (already the basis of cross-cutting/risk-tiers.md) — reused here as the target this domain's data classification feeds into. |
...
- **Schema.org** — a widely-adopted structured-data vocabulary
  ...
  domain's Taxonomies/Ontologies if a generic entity vocabulary is useful,
...
- [x] Modular — independent of the three prior domains' panels, no forced overlap beyond the genuine ISO 27001 connection
- [x] Easy to update — new standard cited later just adds a row
- [x] Easy to maintain — will be reconciled against actual Definitions citations once column 1 is drafted, same check performed for every prior domain
