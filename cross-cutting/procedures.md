...
A layer's own Procedures column (08-procedures.md) references these and
adds only steps genuinely specific to that layer's resources.
...
1. Requester (human or AI Agent/Harness drafting on a human's behalf)
   ...
   required metadata fields (per the owning layer's Metadata Standards).
...
4. On approval, the resource is provisioned and assigned a Resource ID
   per the owning layer's naming convention.
5. On rejection, the Requester is notified with the reason; no resource
   is created.
**Layer-specific extensions:** a layer may add steps to this flow (e.g.,
...
1. Requester or Approver identifies a case where a hard-block policy
   (e.g., CCP-1, CCP-2, or a layer-specific hard-block policy) must be
...
- [x] Modular — layers extend, not duplicate, these base sequences
- [x] Easy to update — one edit updates the flow for every referencing layer
- [x] Easy to maintain — each procedure traces to specific CCP-/CCAR- items
- [x] Easy to replace — no tooling specifics baked in; SLA/notification deferred to Tooling per layer
