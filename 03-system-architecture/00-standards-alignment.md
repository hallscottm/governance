# System Architecture Layer — Standards Alignment
...
Scope per 00-framework/layer-axis-definition.md: databases, applications,
...
layer is about what runs and how it's built/versioned/secured, not the
data's meaning (Data/Metadata, layer 4) or how it's deployed via
pipelines (Workflow/Process, layer 5).
...
| **OWASP ASVS (Application Security Verification Standard)** | OWASP | Application-layer security requirements and verification levels — the app-layer counterpart to Infrastructure/Networking's NIST/CIS coverage |
...
| **OpenAPI Specification (OAI)** | OpenAPI Initiative | Machine-readable API interface description — informs this layer's Conventions and Metadata Standards for service interfaces |
| **Semantic Versioning 2.0.0 (SemVer)** | semver.org (community spec, de facto industry standard) | Version-numbering convention for APIs/services/packages — relevant to this layer's Conventions column |
...
| **ANSI/ISO SQL:2016** | ANSI/ISO/IEC | Relational database language standard — baseline vocabulary for this layer's database-related definitions |
...
- **The Twelve-Factor App** — a widely-adopted industry methodology for
  ...
  vocabulary/conventions for this layer's Conventions column, cited
...
- **DAMA-DMBOK** — not applicable to this layer (data governance content
  belongs to Data/Metadata, layer 4); remains flagged for that layer.
- **ISO/IEC 42001, NIST AI RMF** — not applicable to this layer (AI
  management system / AI risk content); remains flagged for the Harness
  layer where AI-specific governance is more directly relevant.
